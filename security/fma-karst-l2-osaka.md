...
### FM1: EIP-7825 Tx Gas Limit Cap Breaks Existing On-Chain Usage
...
- **Recovery Path(s):** Affected contracts would need to be refactored to use multiple transactions. Deposit transactions are not affected.

### FM2: Osaka EVM Changes Not Correctly Implemented in Fault Proof Programs
...
- **Mitigations:**
  1. End-to-end proof tests exercising each changed opcode/precompile in FP programs.
- **Detection:** op-program trace runner failure, potentially triggering an alert.
- **Recovery Path(s):** 
  1. Emergency switch to the fallback permissioned game type.
  2. Emergency update to fault proof programs to fix the divergent behavior.

### FM3: EIP-7951 secp256r1 Gas Cost Increase Breaks Existing Contracts
...
### FM4: EIP-7883/EIP-7823 MODEXP Changes Break Existing Contracts
