...
- **Contracts**:
    ...
    - Introduction of the `L1BlockInterop`, complementary of `L1Block` contract. It serves to set deposit context to prevent successful calls to `validateMessage`. This feature is called “ban deposits” and is being covered in a separate FMA [here](https://github.com/ethereum-optimism/design-docs/pull/185).
...
- **Description:** A user (or contract) may send a valid cross-chain message, but the final relay step—`relayMessage` or `validateMessage`—never occurs (or is dropped before to gain a safe status) even when the initiated message is included in a finalized block on the origin chain. Since the "ban deposits" feature prevents a user from forcing a transaction to call `validateMessage`, there is no way to ensure `relayMessage` is executed unless it is included by the sequencer in a specific block.
- **Risk Assessment:** High.
