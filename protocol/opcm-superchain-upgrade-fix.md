...
|----------|-------------|------------------|
| Dedicated SuperchainConfig ProxyAdmin | A SuperchainConfig has its own ProxyAdmin that is not shared with any of it's OP chains | When OPCM.upgrade() is called by this SuperchainProxyAdmin, it should check if the SuperchainConfig is already upgraded via a isSuperchainUpgraded mapping. If not upgraded, it should upgrade the SuperchainConfig and set isSuperchainUpgraded for that SuperchainConfig to true. No other contract upgrades would occur since this ProxyAdmin only controls the SuperchainConfig. |
| Shared ProxyAdmin | A SuperchainConfig shares a ProxyAdmin with an OP Chain | When OPCM.upgrade() is called by the shared ProxyAdmin, it should first check if the SuperchainConfig is already upgraded. If not upgraded, it should upgrade both the SuperchainConfig and the OP Chain's contracts. If already upgraded, it should only upgrade the OP Chain's contracts. |
| Non shared ProxyAdmin (SuperchainConfig Already Upgraded) | An OP Chain whose ProxyAdmin is not the Superchain's ProxyAdmin is being upgraded and its SuperchainConfig implementation has already been upgraded to the target version | When OPCM.upgrade() is called by the OP Chain's ProxyAdmin, it should check isSuperchainUpgraded for the SuperchainConfig and see it's true. It should then proceed with upgrading only the OP Chain's contracts. |
| Non shared ProxyAdmin (SuperchainConfig Not Upgraded) | An OP Chain whose ProxyAdmin is not the Superchain's ProxyAdmin is being upgraded but its SuperchainConfig has not been upgraded to the target version yet | When OPCM.upgrade() is called by the OP Chain's ProxyAdmin, it should check isSuperchainUpgraded for the SuperchainConfig. Since it's false, it will attempt to upgrade both. This must revert. |
