...
The `AnchorStateRegistry` would be modified to take over the blacklisting and air-gap roles that
are currently handled by the `OptimismPortal` contract. The `AnchorStateRegistry` would expose
several new getter functions that can be used by the `OptimismPortal` contract to determine if a
given dispute game can be permitted to finalize a withdrawal.
...
result in the game array and to use a small binary search to update the anchor state. A binary
search is likely to be acceptable here because we can bound the starting end of the search range to
the index of the current anchor state and only search over states that finalized after the current
anchor state was finalized. Although this can potentially miss a few eligible anchor states under
certain circumstances, this is an acceptable trade-off as new anchor states *will* eventually be
found after the current anchor state.
...
No changes to the FaultDisputeGame contract would be required.
